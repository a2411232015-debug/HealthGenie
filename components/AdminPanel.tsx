...
export const AdminPanel: React.FC<AdminPanelProps> = ({ meals, onAddMeal, onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newMeal: MealRecommendation = {
      id: `m${Date.now()}`,
      name: form.name,
          ...
          {menuMode === 'ai' ? (
            ...
            <div className={`bg-white p-6 rounded-2xl shadow-sm border transition-all ${reviewMode ? 'border-amber-400 ring-4 ring-amber-50' : 'border-slate-100'}`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <input type="number" placeholder="Protein" value={form.protein} onChange={e => setForm({...form, protein: e.target.value})} className="w-full p-1.5 text-center text-sm border border-slate-200 rounded-lg" />
                    </div>
                    <div>
                      <span className="text-xs text-emerald-500 font-bold block mb-1">脂肪</span>
                      <input type="number" placeholder="Fat" value={form.fat} onChange={e => setForm({...form, fat: e.target.value})} className="w-full p-1.5 text-center text-sm border border-slate-200 rounded-lg" />
                    </div>
                    <div>
                      <span className="text-xs text-amber-500 font-bold block mb-1">碳水</span>
                      <input type="number" placeholder="Carbs" value={form.carbs} onChange={e => setForm({...form, carbs: e.target.value})} className="w-full p-1.5 text-center text-sm border border-slate-200 rounded-lg" />
                    </div>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  className={`w-full py-3 rounded-xl font-bold transition-all mt-2 shadow-lg ${
                    reviewMode 
                    ? 'bg-amber-500 hover:bg-amber-600 text-white shadow-amber-500/30' 
                    : 'bg-slate-800 hover:bg-slate-900 text-white shadow-slate-800/30'
                  }`}
                >
                  {reviewMode ? '確認無誤，立即上架' : '確認上架'}
                </button>
              </form>
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col h-[600px] overflow-hidden">
          ...
          <div className="overflow-y-auto flex-1 p-2 space-y-2">
            {meals.slice().reverse().map((meal, index) => (
              <div key={meal.id} className="flex gap-3 items-start p-3 bg-white hover:bg-slate-50 rounded-xl border border-slate-100 group transition-colors">
